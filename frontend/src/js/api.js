...
let _authenticated = null; // null=unbekannt, true=eingeloggt, false=ausgeloggt

export function setAuthState(val) { _authenticated = val; }
export function getAuthState()    { return _authenticated; }
...
export const api = {
  ...
  me:            async () => {
    ...
    if (user && (user.theme === 'light' || user.theme === 'dark')) {
      localStorage.setItem('ff_theme', user.theme);
      if (user.theme === 'dark') document.documentElement.setAttribute('data-theme', 'dark');
  ...
  logout:        async () => { const r = await request('POST', '/auth/logout'); _authenticated = false; return r; },
  ...
  // Lagerorte
  getStorageLocations:    ()          => request('GET',    '/storage-locations'),
  createStorageLocation:  (body)      => request('POST',   '/storage-locations', body),
  updateStorageLocation:  (id, body)  => request('PUT',    `/storage-locations/${id}`, body),
  deleteStorageLocation:  (id)        => request('DELETE', `/storage-locations/${id}`),
  ...
  getArticles: (params) => {
    ...
    const s = qs.toString();
    return request('GET', s ? `/articles?${s}` : '/articles');
  },
  ...
  lowStock:      ()      => request('GET',    '/articles/low-stock'),
  lookupEan:     (ean)   => request('GET',    `/articles/ean/${encodeURIComponent(ean)}`),

  // Chargen
  getCharges:    (aid)        => request('GET',    `/articles/${aid}/charges`),
  createCharge:  (aid, body)  => request('POST',   `/articles/${aid}/charges`, body),
  updateCharge:  (aid, cid, b)=> request('PUT',    `/articles/${aid}/charges/${cid}`, b),
  deleteCharge:  (aid, cid)   => request('DELETE', `/articles/${aid}/charges/${cid}`),

  getArticleInspections:    (aid)         => request('GET',    `/articles/${aid}/inspections`),
  createArticleInspection:  (aid, body)   => request('POST',   `/articles/${aid}/inspections`, body),
  updateArticleInspection:  (aid, iid, b) => request('PUT',    `/articles/${aid}/inspections/${iid}`, b),
  deleteArticleInspection:  (aid, iid)    => request('DELETE', `/articles/${aid}/inspections/${iid}`),

  getArticleInstances:    (aid)         => request('GET',    `/articles/${aid}/instances`),
  createArticleInstance:  (aid, body)   => request('POST',   `/articles/${aid}/instances`, body),
  updateArticleInstance:  (aid, iid, b) => request('PUT',    `/articles/${aid}/instances/${iid}`, b),
  deleteArticleInstance:  (aid, iid)    => request('DELETE', `/articles/${aid}/instances/${iid}`),

  getScanData:             (token)        => request('GET',    `/scan/${token}`),

  getArticleCategories:    ()             => request('GET',    '/articles/categories'),
  createArticleCategory:   (body)         => request('POST',   '/articles/categories', body),
  updateArticleCategory:   (id, body)     => request('PUT',    `/articles/categories/${id}`, body),
  deleteArticleCategory:   (id)           => request('DELETE', `/articles/categories/${id}`),

  getArticleComponents:    (aid)          => request('GET',    `/articles/${aid}/components`),
  createArticleComponent:  (aid, body)    => request('POST',   `/articles/${aid}/components`, body),
  updateArticleComponent:  (aid, cid, b)  => request('PUT',    `/articles/${aid}/components/${cid}`, b),
  deleteArticleComponent:  (aid, cid)     => request('DELETE', `/articles/${aid}/components/${cid}`),
  ...
  setBadge:       (id, code)  => request('PUT',    `/admin/users/${id}/badge`, { badge_code: code }),
  getAuditLog:      ()        => request('GET',    '/admin/audit-log'),
  getContainerLog:  ()        => request('GET',    '/admin/container-log'),
  checkForUpdate:   ()        => request('GET',    '/admin/update-check'),
  ...
  clockPunch:           (body)  => request('POST',  '/clock/punch', body),
  clockActive:          ()      => request('GET',   '/clock/active'),
  timeclockHistory:     (from, to) => request('GET', `/timeclock/history?from=${from}&to=${to}`),
  timeclockSummary:     (from, to) => request('GET', `/timeclock/summary?from=${from}&to=${to}`),

  // Mein Bereich (Selfservice)
  exportMyData:        ()          => request('GET',    '/me/export'),
  ...
  returnMyEquipment:   (id)        => request('PUT',    `/me/equipment/${id}/return`),
  ...
  // iCal Kalender-Sync
  getIcalToken:        ()             => request('GET',    '/me/ical-token'),
  generateIcalToken:   ()             => request('POST',   '/me/ical-token'),
  revokeIcalToken:     ()             => request('DELETE', '/me/ical-token'),
  ...
  updateVehicleType:    (id, body)      => request('PUT',    `/vehicles/types/${id}`, body),
  deleteVehicleType:    (id)            => request('DELETE', `/vehicles/types/${id}`),
  ...
  downloadDocument: (id) => {
    return fetch(`${BASE}/verein/dokumente/${id}/download`, { credentials: 'include' });
  },
  ...
  exportEventCsv:          (id)       => {
    return fetch(`${BASE}/verein/events/${id}/antworten/csv`, { credentials: 'include' });
  },
  ...
  exportIncidentStats: (year, format) => {
    const p = new URLSearchParams();
    if (year)   p.set('year', year);
    if (format) p.set('format', format);
    return `${BASE}/einsatzberichte/stats/export?${p}`;
  },
  ...
  updateIncidentPersonnel: (id, pid, body)=> request('PUT',    `/einsatzberichte/${id}/personal/${pid}`, body),
  ...
  downloadIncidentAttachment: (id, aid) => {
    return fetch(`${BASE}/einsatzberichte/${id}/anhaenge/${aid}/download`, { credentials: 'include' });
  },
  ...
  createDienstReport:  (body)   => request('POST',   '/dienstberichte', body),
  updateDienstReport:  (id, b)  => request('PUT',    `/dienstberichte/${id}`, b),
  deleteDienstReport:  (id)     => request('DELETE', `/dienstberichte/${id}`),

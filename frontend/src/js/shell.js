...
const PAGE_MODULE = {
  ...
  dienstberichte:    'einsatzberichte',
  personal:        'personal',
  termine:         'personal',
...
const MODULE_COLORS = {
  ...
  fahrzeugpruefung: { c: '#0891b2',           hell: '#06b6d41a' },
...
function buildShell() {
  ...
  const lagerItems = [
    { page: 'orders',     label: 'Bestellübersicht', icon: icon('clipboard-list', 16) },
    ...(canAccess(currentUser, 'lager') || canAccess(currentUser, 'lager.approve') ? [
      { page: 'new-order', label: 'Neue Bestellung',  icon: icon('plus', 16) },
      { page: 'articles',  label: 'Artikelstamm',     icon: icon('package', 16) },
    ] : []),
  ];

  const personalItems = [
    { page: 'personal',      label: 'Mitglieder',   icon: icon('users', 16) },
    { page: 'termine',       label: 'Termine',       icon: icon('calendar', 16) },
    { page: 'zeiterfassung', label: 'Zeiterfassung', icon: icon('clock', 16) },
  ];
  ...
  const einsaetzeItems = [
    ...
    ...(canAccess(currentUser, 'einsatzberichte') ? [
      { page: 'new-incident', label: 'Neuer Bericht',   icon: icon('plus', 16) },
    ] : []),
  ];
...
function bindEvents(app) {
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', async () => {
      try { await api.logout(); } catch (_) { /* Logout-Fehler ignorieren */ }
      navigate('#/login');
    });
  }
